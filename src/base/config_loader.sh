...
load_config "dag_run_conf_overrides_params" "${CORE_DAG_RUN_CONF_OVERRIDES_PARAMS:=True}" "airflow.cfg"
...
load_config_with_opt "sql_alchemy_conn" "${AIRFLOW_DATABASE_PASSWORD:=NULL}" "${AIRFLOW_DATABASE_TYPE:=postgresql}://${AIRFLOW_DATABASE_USER:=NULL}:${AIRFLOW_DATABASE_PASSWORD:=NULL}@${AIRFLOW_DATABASE_HOST:=NULL}:${AIRFLOW_DATABASE_PORT:=${__SERVICE_PORTS__[${AIRFLOW_DATABASE_TYPE:=postgresql}]}}/${AIRFLOW_DATABASE_NAME:=NULL}" "${AIRFLOW_DATABASE_TYPE:=postgresql}://${AIRFLOW_DATABASE_HOST:=NULL}:${AIRFLOW_DATABASE_PORT:=${__SERVICE_PORTS__[${AIRFLOW_DATABASE_TYPE:=postgresql}]}}/${AIRFLOW_DATABASE_NAME:=NULL}" "airflow.cfg"
...
load_config "sql_alchemy_connect_args" "${CORE_SQL_ALCHEMY_CONNECT_ARGS:=NULL}" "airflow.cfg"
load_config "sql_engine_encoding" "${CORE_SQL_ENGINE_ENCODING:=utf-8}" "airflow.cfg"
load_config "sql_engine_collation_for_ids" "${CORE_SQL_ENGINE_COLLATION_FOR_IDS:=NULL}" "airflow.cfg"
...
load_config "max_num_rendered_ti_fields_per_task" "${CORE_MAX_NUM_RENDERED_TI_FIELDS_PER_TASK:=30}" "airflow.cfg"
load_config "max_db_retries" "${CORE_MAX_DB_RETRIES:=3}" "airflow.cfg"
load_config "load_examples" "${CORE_LOAD_EXAMPLES:=False}" "airflow.cfg"
load_config "load_default_connections" "${CORE_LOAD_DEFAULT_CONNECTIONS:=False}" "airflow.cfg"
...
load_config "dagbag_import_error_tracebacks" "${CORE_DAGBAG_IMPORT_ERROR_TRACEBACKS:=True}" "airflow.cfg"
load_config "dagbag_import_error_traceback_depth" "${CORE_DAGBAG_IMPORT_ERROR_TRACEBACK_DEPTH:=2}" "airflow.cfg"
...
load_config "store_dag_code" "${CORE_STORE_DAG_CODE:=NULL}" "airflow.cfg"
load_config "min_serialized_dag_update_interval" "${CORE_MIN_SERIALIZED_DAG_UPDATE_INTERVAL:=30}" "airflow.cfg"
load_config "min_serialized_dag_fetch_interval" "${CORE_MIN_SERIALIZED_DAG_FETCH_INTERVAL:=10}" "airflow.cfg"
load_config "check_slas" "${CORE_CHECK_SLAS:=True}" "airflow.cfg"
load_config "execute_tasks_new_python_interpreter" "${CORE_EXECUTE_TASKS_NEW_PYTHON_INTERPRETER:=False}" "airflow.cfg"
load_config "xcom_backend" "${CORE_XCOM_BACKEND:=airflow.models.xcom.BaseXCom}" "airflow.cfg"
load_config "lazy_load_plugins" "${CORE_LAZY_LOAD_PLUGINS:=True}" "airflow.cfg"
load_config "lazy_discover_providers" "${CORE_LAZY_DISCOVER_PROVIDERS:=True}" "airflow.cfg"
...
printf "\n[api]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "enable_experimental_api" "${API_ENABLE_EXPERIMENTAL_API:=False}" "airflow.cfg"
load_config "auth_backend" "${API_AUTH_BACKEND:=airflow.api.auth.backend.default}" "airflow.cfg"
load_config "maximum_page_limit" "${API_MAXIMUM_PAGE_LIMIT:=100}" "airflow.cfg"
load_config "fallback_page_limit" "${API_FALLBACK_PAGE_LIMIT:=100}" "airflow.cfg"
load_config "google_oauth2_audience" "${API_GOOGLE_OAUTH2_AUDIENCE:=NULL}" "airflow.cfg"
load_config "google_key_path" "${API_GOOGLE_KEY_PATH:=NULL}" "airflow.cfg"

printf "\n[lineage]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "backend" "${LINEAGE_BACKEND:=NULL}" "airflow.cfg"
...
load_config "default_allow_illegal_arguments" "${OPERATORS_DEFAULT_ALLOW_ILLEGAL_ARGUMENTS:=False}" "airflow.cfg"

printf "\n[hive]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "default_hive_mapred_queue" "${HIVE_DEFAULT_HIVE_MAPRED_QUEUE:=NULL}" "airflow.cfg"
load_config "mapred_job_name_template" "${HIVE_MAPRED_JOB_NAME_TEMPLATE:=NULL}" "airflow.cfg"
...
load_config "expose_hostname" "${WEBSERVER_EXPOSE_HOSTNAME:=True}" "airflow.cfg"
load_config "expose_stacktrace" "${WEBSERVER_EXPOSE_STACKTRACE:=True}" "airflow.cfg"
...
load_config "log_fetch_delay_sec" "${WEBSERVER_LOG_FETCH_DELAY_SEC:=2}" "airflow.cfg"
load_config "log_auto_tailing_offset" "${WEBSERVER_LOG_AUTO_TAILING_OFFSET:=30}" "airflow.cfg"
load_config "log_animation_speed" "${WEBSERVER_LOG_ANIMATION_SPEED:=1000}" "airflow.cfg"
...
load_config "default_ui_timezone" "${WEBSERVER_DEFAULT_UI_TIMEZONE:=UTC}" "airflow.cfg"
...
load_config "reload_on_plugin_change" "${WEBSERVER_RELOAD_ON_PLUGIN_CHANGE:=False}" "airflow.cfg"
load_config "proxy_fix_x_for" "${WEBSERVER_PROXY_FIX_X_FOR:=1}" "airflow.cfg"
load_config "proxy_fix_x_proto" "${WEBSERVER_PROXY_FIX_X_PROTO:=1}" "airflow.cfg"
load_config "proxy_fix_x_host" "${WEBSERVER_PROXY_FIX_X_HOST:=1}" "airflow.cfg"
load_config "proxy_fix_x_port" "${WEBSERVER_PROXY_FIX_X_PORT:=1}" "airflow.cfg"
load_config "proxy_fix_x_prefix" "${WEBSERVER_PROXY_FIX_X_PREFIX:=1}" "airflow.cfg"
load_config "x_frame_enabled" "${WEBSERVER_X_FRAME_ENABLED:=True}" "airflow.cfg"
load_config "analytics_tool" "${WEBSERVER_ANALYTICS_TOOL:=NULL}" "airflow.cfg"
load_config "analytics_id" "${WEBSERVER_ANALYTICS_ID:=NULL}" "airflow.cfg"
load_config "update_fab_perms" "${WEBSERVER_UPDATE_FAB_PERMS:=True}" "airflow.cfg"
load_config "session_lifetime_minutes" "${WEBSERVER_SESSION_LIFETIME_MINUTES:=43200}" "airflow.cfg"

printf "\n[email]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "email_backend" "${EMAIL_EMAIL_BACKEND:=airflow.utils.email.send_email_smtp}" "airflow.cfg"
load_config "default_email_on_retry" "${EMAIL_DEFAULT_EMAIL_ON_RETRY:=True}" "airflow.cfg"
load_config "default_email_on_failure" "${EMAIL_DEFAULT_EMAIL_ON_FAILURE:=True}" "airflow.cfg"
load_config "subject_template" "${EMAIL_SUBJECT_TEMPLATE:=NULL}" "airflow.cfg"
load_config "html_content_template" "${EMAIL_HTML_CONTENT_TEMPLATE:=NULL}" "airflow.cfg"
...
load_config "smtp_timeout" "${SMTP_SMTP_TIMEOUT:=30}" "airflow.cfg"
load_config "smtp_retry_limit" "${SMTP_SMTP_RETRY_LIMIT:=5}" "airflow.cfg"

printf "\n[sentry]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "sentry_on" "${SENTRY_SENTRY_ON:=false}" "airflow.cfg"
...
load_config "worker_autoscale" "${CELERY_WORKER_AUTOSCALE:=NULL}" "airflow.cfg"
load_config "worker_prefetch_multiplier" "${CELERY_WORKER_PREFETCH_MULTIPLIER:=NULL}" "airflow.cfg"
load_config "worker_log_server_port" "${CELERY_WORKER_LOG_SERVER_PORT:=8793}" "airflow.cfg"
load_config "worker_umask" "${CELERY_WORKER_UMASK:=0o077}" "airflow.cfg"
load_config "worker_precheck" "${CELERY_WORKER_PRECHECK:=False}" "airflow.cfg"
...
load_config "operation_timeout" "${CELERY_OPERATION_TIMEOUT:=1.0}" "airflow.cfg"
load_config "task_track_started" "${CELERY_TASK_TRACK_STARTED:=True}" "airflow.cfg"
load_config "task_adoption_timeout" "${CELERY_TASK_ADOPTION_TIMEOUT:=600}" "airflow.cfg"
load_config "task_publish_max_retries" "${CELERY_TASK_PUBLISH_MAX_RETRIES:=3}" "airflow.cfg"
...
load_config "min_file_process_interval" "${SCHEDULER_MIN_FILE_PROCESS_INTERVAL:=30}" "airflow.cfg"
...
load_config "max_dagruns_to_create_per_loop" "${SCHEDULER_MAX_DAGRUNS_TO_CREATE_PER_LOOP:=NULL}" "airflow.cfg"
load_config "max_dagruns_peer_loop_to_schedule" "${SCHEDULER_MAX_DAGRUNS_PEER_LOOP_TO_SCHEDULE:=NULL}" "airflow.cfg"
...
load_config "allow_trigger_in_future" "${SCHEDULER_ALLOW_TRIGGER_IN_FUTURE:=False}" "airflow.cfg"
load_config "pool_metrics_interval" "${SCHEDULER_POOL_METRICS_INTERVAL:=5.0}" "airflow.cfg"
load_config "orphaned_tasks_check_interval" "${SCHEDULER_ORPHANED_TASKS_CHECK_INTERVAL:=300.0}" "airflow.cfg"
load_config "use_row_level_locking" "${SCHEDULER_USE_ROW_LEVEL_LOCKING:=True}" "airflow.cfg"
load_config "schedule_after_task_execution" "${SCHEDULER_SCHEDULE_AFTER_TASK_EXECUTION:=NULL}" "airflow.cfg"
load_config "parsing_processes" "${SCHEDULER_PARSING_PROCESSES:=2}" "airflow.cfg"
...
load_config "sensitive_variable_fields" "${ADMIN_SENSITIVE_VARIABLE_FIELDS:=NULL}" "airflow.cfg"
...
printf "\n[kubernetes]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "pod_template_file" "${KUBERNETES_POD_TEMPLATE_FILE:=NULL}" "airflow.cfg"
...
load_config "delete_worker_pods" "${KUBERNETES_DELETE_WORKER_PODS_:=True}" "airflow.cfg"
load_config "delete_worker_pods_on_failure" "${KUBERNETES_DELETE_WORKER_PODS_ON_FAILURE:=False}" "airflow.cfg"
load_config "multi_namespace_mode" "${KUBERNETES_MULTI_NAMESPACE_MODE:=False}" "airflow.cfg"
load_config "in_cluster" "${KUBERNETES_IN_CLUSTER:=True}" "airflow.cfg"
load_config "cluster_context" "${KUBERNETES_CLUSTER_CONTEXT:=NULL}" "airflow.cfg"
load_config "config_file" "${KUBERNETES_CONFIG_FILE:=NULL}" "airflow.cfg"
load_config "kube_client_request_args" "${KUBERNETES_KUBE_CLIENT_REQUEST_ARGS:=NULL}" "airflow.cfg"
load_config "delete_option_kwargs" "${KUBERNETES_DELETE_OPTION_KWARGS:=NULL}" "airflow.cfg"
load_config "enable_tcp_keepalive" "${KUBERNETES_ENABLE_TCP_KEEPALIVE:=False}" "airflow.cfg"
load_config "tcp_keep_idle" "${KUBERNETES_TCP_KEEP_IDLE:=120}" "airflow.cfg"
load_config "tcp_keep_intvl" "${KUBERNETES_TCP_KEEP_INTVL:=30}" "airflow.cfg"
load_config "tcp_keep_cnt" "${KUBERNETES_TCP_KEEP_CNT:=6}" "airflow.cfg"

printf "\n[smart_sensor]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "use_smart_sensor" "${SMART_SENSOR_USE_SMART_SENSOR:=False}" "airflow.cfg"
load_config "shard_code_upper_limit" "${SMART_SENSOR_SHARD_CODE_UPPER_LIMIT:=10000}" "airflow.cfg"
load_config "shards" "${SMART_SENSOR_SHARDS:=5}" "airflow.cfg"
load_config "sensors_enabled" "${SMART_SENSOR_SENSORS_ENABLED:=NamedHivePartitionSensor}" "airflow.cfg"

printf "\n[secrets]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "backend" "${SECRETS_BACKEND_:=}" "airflow.cfg"
load_config "backend_kwargs" "${SECRETS_BACKEND_KWARGS:=}" "airflow.cfg"

printf "\n[logging]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "base_log_folder" "${AIRFLOW_HOME}/logs" "airflow.cfg"
load_config "remote_logging" "${LOGGING_REMOTE_LOGGING:=False}" "airflow.cfg"
load_config "remote_log_conn_id" "${LOGGING_REMOTE_LOG_CONN_ID:=NULL}" "airflow.cfg"
load_config "remote_base_log_folder" "${LOGGING_REMOTE_BASE_LOG_FOLDER:=NULL}" "airflow.cfg"
load_config "google_key_path" "${LOGGING_GOOGLE_KEY_PATH:=NULL}" "airflow.cfg"
load_config "encrypt_s3_logs" "${LOGGING_ENCRYPT_S3_LOGS:=False}" "airflow.cfg"
load_config "logging_level" "${LOGGING_LOGGING_LEVEL:=INFO}" "airflow.cfg"
load_config "logging_config_class" "${LOGGING_LOGGING_CONFIG_CLASS:=NULL}" "airflow.cfg"
load_config "fab_logging_level" "${LOGGING_FAB_LOGGING_LEVEL:=WARN}" "airflow.cfg"
load_config "colored_console_log" "${LOGGING_COLORED_CONSOLE_LOG:=True}" "airflow.cfg"
load_config "colored_log_format" "${LOGGING_COLORED_LOG_FORMAT:=[%%%%(blue)s%%%%(asctime)s%%%%(reset)s] {{%%%%(blue)s%%%%(filename)s:%%%%(reset)s%%%%(lineno)d\}\} %%%%(log_color)s%%%%(levelname)s%%%%(reset)s - %%%%(log_color)s%%%%(message)s%%%%(reset)s}" "airflow.cfg"
load_config "colored_formatter_class" "${LOGGING_COLORED_FORMATTER_CLASS:=airflow.utils.log.colored_log.CustomTTYColoredFormatter}" "airflow.cfg"
load_config "log_format" "${LOGGING_LOG_FORMAT:=[%%%%(asctime)s] {{%%%%(filename)s:%%%%(lineno)d\}\} %%%%(levelname)s - %%%%(message)s}" "airflow.cfg"
load_config "log_filename_template" "${LOGGING_LOG_FILENAME_TEMPLATE:={{ ti.dag_id \}\}/{{ ti.task_id \}\}/{{ ts \}\}/{{ try_number \}\}.log}" "airflow.cfg"
load_config "log_processor_filename_template" "${LOGGING_LOG_PROCESSOR_FILENAME_TEMPLATE:={{ filename \}\}.log}" "airflow.cfg"
load_config "simple_log_format" "${LOGGING_SIMPLE_LOG_FORMAT:=%%%%(asctime)s %%%%(levelname)s - %%%%(message)s}" "airflow.cfg"
...
load_config "task_log_reader" "${LOGGING_TASK_LOG_READER:=task}" "airflow.cfg"
load_config "task_log_prefix_template" "${LOGGING_TASK_LOG_PREFIX_TEMPLATE:=NULL}" "airflow.cfg"
load_config "extra_loggers" "${LOGGING_EXTRA_LOGGERS:=NULL}" "airflow.cfg"

printf "\n[metrics]\n" >> "${AIRFLOW_CONF_DIR}/airflow.cfg"
load_config "statsd_on" "${METRICS_STATSD_ON:=False}" "airflow.cfg"
load_config "statsd_host" "${METRICS_STATSD_HOST:=localhost}" "airflow.cfg"
load_config "statsd_port" "${METRICS_STATSD_PORT:=8125}" "airflow.cfg"
load_config "statsd_prefix" "${METRICS_STATSD_PREFIX:=airflow}" "airflow.cfg"
load_config "statsd_allow_list" "${METRICS_STATSD_ALLOW_LIST:=NULL}" "airflow.cfg"
load_config "statsd_datadog_enabled" "${METRICS_STATSD_DATADOG_ENABLED:=False}" "airflow.cfg"
load_config "statsd_datadog_tags" "${METRICS_STATSD_DATADOG_TAGS:=NULL}" "airflow.cfg"
load_config "statsd_custom_client_path" "${METRICS_STATSD_CUSTOM_CLIENT_PATH:=NULL}" "airflow.cfg"
load_config "stat_name_handler" "${METRICS_STAT_NAME_HANDKLER:=NULL}" "airflow.cfg"
